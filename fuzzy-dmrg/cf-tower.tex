...
\begin{document}
\begin{tikzpicture}[x=1.6cm, y=1.6cm, >=Stealth, font=\sffamily]

    % ==========================================
    % 1. BACKGROUND GRID & CONE
    % ==========================================
    % Grid
    \draw[step=1.0, gray!20, thin, dashed] (-5.5, 0) grid (5.5, 6.5);
    
    % Cone Fill
    \fill[yellow!10, opacity=0.7] (0,1) -- (5.5, 6.5) -- (-5.5, 6.5) -- cycle;
    
    % Cone Boundaries (Unitarity-like bounds for this module)
    \draw[thick, dashed, black!60] (0,1) -- (5.5, 6.5) node[midway, sloped, above] {$E = J + \Delta_0$};
    \draw[thick, dashed, black!60] (0,1) -- (-5.5, 6.5) node[midway, sloped, above] {$E = -J + \Delta_0$};
    ...
    \draw[->, line width=1.2pt] (-5.8, 0) -- (6.2, 0) node[right, align=center] {\textbf{Angular Momentum $J$}\\ \small($s = h - \bar{h}$)};
    
    % Y-axis : Energy / Scaling Dimension
    \draw[->, line width=1.2pt] (0, -0.2) -- (0, 7) node[above, align=center] {\textbf{Scaling Dimension $\Delta$ (Energy)}\\ \small($E = h + \bar{h}$)};

    % Axis Tick Marks
    \draw (5, 0.1) -- (5, -0.1) node[below=2pt] {$5$};
    \draw (-5, 0.1) -- (-5, -0.1) node[below=2pt] {$-5$};
    \node[below left=1pt] at (0,0) {$0$};
    \draw (0.1, 5) -- (-0.1, 5) node[left=4pt] {$5$};
    ...
    \draw[<-] (N_0_0) ++(-0.2, -0.3) to[bend right=20] (-1.5, -0.6) node[left] {Highest Weight State};
