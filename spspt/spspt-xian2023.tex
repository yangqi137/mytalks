...
\item A more general construction for space-group SPTs w/ all possible $G$.
\end{enumerate}
\begin{center}
...
\begin{frame}
	...
	\begin{columns}
      ...
      \item<4-> Anomaly = boundary $\partial\hat\omega$.
		\end{itemize}
		\column{.42\textwidth}
		\begin{tikzpicture}
...
\column{.6\textwidth}
\begin{itemize}
\item A Chern-insulator bubble creates a trivial decoration.
\item How to compute:
\begin{enumerate}
	\item $\hat\mu = \sigma$ labels a bubble around $\sigma$.
	\item The decoration it creates:
	$\tau_1-\tau_2+\tau_3-\tau_4+\tau_5-\tau_6=\partial\sigma$.
\end{enumerate}
\item Therefore, $\forall\hat\mu\in C_3(X)$, $\partial\hat\mu$ is trivial.
\end{itemize}
\column{.4\textwidth}
...
\includegraphics[width=3cm]{cell3}
\end{columns}
\end{frame}
...
\begin{itemize}
\item $SG=\mathbb Z^3$, $X'=T^3=\mathbb R^3/SG$.
...
\begin{frame}
	...
	\begin{columns}
		...
		\fill [red] (1,1) circle (2pt);
		\node at (-.5, -.5) {$\alpha$};
		\node at (.5, -.5) {$t_x\cdot\alpha$};
		\node at (-.5, .5) {$t_y\cdot\alpha$};
		\end{tikzpicture}
		\end{center}
		\column{.8\textwidth}
...
\begin{frame}
	...
	\begin{columns}
		...
		\column{.8\textwidth}
		\begin{enumerate}
			\item Between $\alpha$ and $t_x\cdot\alpha$: put $\gamma_1=0$.
			\item Between $\alpha$ and $t_y\cdot\alpha$:
			$[\alpha]\simeq[t_y\cdot\alpha]$ but $\alpha\neq t_y\cdot\alpha $
			\item Choose $\gamma_2$ s.t. $d\gamma_2 = \alpha-t_y\cdot\alpha$.
			\[\gamma_2(g_0,g_1,g_2)=n_{tx}(g_0)\beta(g_0,g_1,g_2).\]
			\item $\gamma_4=t_x\cdot \gamma_2$:
			\[\gamma_4(g_0,g_1,g_2)=[n_{tx}(g_0)+1]\beta(g_0,g_1,g_2).\]
			$\gamma_3=t_y\cdot\gamma_1=0$.
			\item $\gamma_1+\gamma_2+\gamma_3+\gamma_4=\beta$.
		\end{enumerate}
	\end{columns}
\end{frame}
