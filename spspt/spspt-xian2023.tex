...
\begin{itemize}
\item Zhi-Da Song (宋志达): Peking University.
\item Chen Fang (方辰): Institute of Physics, Beijing.
%\item Jian-Hao Zhang: 
\begin{center}
	\includegraphics[height=3cm]{../people/zhidasong}
	\includegraphics[height=3cm]{../people/chenfang}
\end{center}
\item Zhida Song, Chen Fang and Yang Qi, Nat. Commun. 11, 4197 (2020).
\end{itemize}
\end{frame}
...
\begin{frame}{Topological phases}
	...
	\begin{center}
		\begin{tikzpicture}
			\draw[fill=blue!40] (0, 0)--(4, 0)--(4, 2)--(0, 2)--(0, 0);
			\fill[red] (1, 1) circle(5pt);
			\fill[red] (3, 1) circle(5pt);
		\end{tikzpicture}
		\hspace{1cm}
		\begin{tikzpicture}
			\draw[fill=blue!40] (0, 0)--(4, 0)--(4, 2)--(0, 2)--(0, 0);
			\draw[-latex=5pt](0, 0)--(2.1, 0);
		\end{tikzpicture}
	\end{center}
\end{frame}
...
\begin{enumerate}
	...
	\item Bosonic SPTs: Haldane chain, CZX/Levin-Gu state, etc; group-cohomology theory.
...
\begin{frame}
  ...
  Classification of onsite SPT is (almost) solved. How about space-group symmetries?
  \begin{enumerate}
  \item Thorngren and Else (2018): the classification for onsite symmetry $G$ and space-group symmetry $G$ is the same.
    \begin{itemize}
    \item Symmetries that reverses oritation (such as mirror reflection and glide planes) maps to antiunitary symmetries.
    \item Spin-1/2 fermions map to spinless fermions.
    \end{itemize}
  \item Dimensional reduction: Liang Fu, Michael Hermele et al.\\
    \emph{Examples: mirror SPT, weak SPT (translation symmetry).}
  \end{enumerate}
  Our work: A more general construction for space-group SPTs w/ all possible $G$.
  \begin{center}
...
\section{Recipe: 1st and 2nd pages.}
...
\begin{frame}
	...
	\end{enumerate}
	A $G$-complex $Y\simeq \mathbb R^d$.
	\begin{columns}
		\column{.5\textwidth}
		\begin{center}
...
\begin{frame}
	...
	\begin{columns}
		...
		\begin{itemize}
			...
			\[\hat\omega(\sigma)\in \Phi^p(G_\sigma).\]
...
\begin{frame}
	...
	\begin{columns}
		...
		\begin{itemize}
			...
			\[\Phi^p(G_{\sigma'})\simeq \Phi^p(G_\sigma)\]
...
\begin{frame}
	...
	\begin{columns}
		...
		\begin{itemize}
			...
			\item<4-> Hence, $\hat\omega(\sigma_i)$ is not $G_\tau$-symmetric.
			\item<4-> When $[G_\tau:G_\sigma]>1$: $\tau$ borders $[G_\tau:G_\sigma]$ symmetry-related copies of $\sigma_i$.
			\item<4-> $\hat\omega(\sigma_1)
...
\begin{frame}
	...
	\begin{columns}
		...
		\begin{itemize}
			...
			+\hat\omega(\sigma_2)+\hat\omega(\sigma_3)+\hat\omega(\sigma_4).\]
			\item Anomaly cancelation: $\partial\omega\simeq0.$
...
\begin{frame}
  \frametitle{SPT Classification as (Generalized) Cohomology Theories}
  \begin{itemize}
  \item Phases $\Phi^d(G)$ are equivalence class of wave functions $\Psi^d(G)$.
  \item Bulk-boundary relation: $d\alpha=\beta, \alpha\in \Psi^d(G), \beta\in \Psi^{d+1}(G).$
  \item Coboundary operator: $d^d: \Psi^d(G)\rightarrow\Psi^{d+1}(G).$
  \item Two understandings:
    \begin{enumerate}
    \item Cocycle condition: $\alpha$ represents a $d$-dim topological state iff $d\alpha=0$.\\
      $\text{Cocycles}=\ker d^d.$
    \item Coboundary equivalence: $d\alpha$ is a trivial state; $\beta\sim\beta+d\alpha$.\\
      $\text{Coboundaries}=\img d^{d-1}.$
    \end{enumerate}
  \item $d^2=0$: $\img d^{d-1}\subset\ker d^d$.
  \item A cochain complex:$\cdots\rightarrow\Psi^{d-1}(G)\xrightarrow{d^{d-1}}\Psi^d(G)\xrightarrow{d^d}\Psi^{d+1}(G)\rightarrow\cdots,$
  \item Cohomology theory: $\Phi^d(G)=\mathcal H^{d+1}(G)=\frac{\ker d^d}{\img d^{d-1}}.$
  \end{itemize}
  \begin{center}
    \begin{tikzpicture}
      \fill (.5, .5)--(4.5, 0.5)--(5, 1)--(5, 2)--(1, 2)--(0.5, 1.5)--(0.5, 0.5) [mbg!15];
      \node at (2.75, 1.5) {$\beta$};
      \fill (0.5, 0.5)--(4.5, 0.5)--(5, 1)--(1, 1)--(0.5, 0.5) [mbg];
      \node  [text=white] at (2.75, 0.75) {$\alpha$};
...
\begin{frame}
  ...
  \begin{columns}
    ...
    \item $\hat\omega(\sigma_1)
      +\hat\omega(\sigma_2)+\hat\omega(\sigma_3)+\hat\omega(\sigma_4)\in \Psi^p(G_\tau)$.
    \item Generalize the boundary-transfer operation $\partial\hat\omega$:
      \[(\partial\hat\omega)(\tau)=\hat\omega(\sigma_1)
        +\hat\omega(\sigma_2)+\hat\omega(\sigma_3)+\hat\omega(\sigma_4).\]
    \item Bulk-Boundary relation: $d\hat\omega - \partial\hat\omega=0.$
    \end{itemize}
    \column{.42\textwidth}
    \begin{tikzpicture}
      \draw (0, 1)--(-1, 1)--(-2, 0)--(2, 0)--(3, 1)--(1, 1);
...
\begin{itemize}
\item Another work about spectral sequence: Ken Shiozaki, Masatoshi Sato and Kiyonori Gomi, PRB 106, 165103 (2022) (momentum-space analysis).
\item An independent work: Else and Thorngren, PRB 99, 115116 (2019).
...
\begin{frame}
  ...
  \begin{itemize}
		\item 2D point groups: Jian-Hao Zhang (张健豪), Qing-Rui Wang, Shuo Yang, YQ, and Zheng-Cheng Gu, PRB 101, 100501 (2020).
  \item 2D wallpaper groups: J-H Zhang, S Yang, YQ and Z-C Gu, PR Research 4, 033081 (2022).
  \item 3D point groups: J-H Zhang, YQ and Z-C Gu, arXiv:2204.13558.
...
\begin{enumerate}
\item High-order topological bounary states: real-space construction w/ boundaries.\\
YQ, Chen Fang, to appear.
